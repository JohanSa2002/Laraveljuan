                    ...
                    <form action="{{ route('admin.notices.store') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="mb-4">
                            ...
                            <select name="category" id="category" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                <option value="Aviso" {{ old('category') == 'Aviso' ? 'selected' : '' }}>Aviso Oficial</option>
                                <option value="Academia" {{ old('category') == 'Academia' ? 'selected' : '' }}>Academia (Calendario, Admisión)</option>
                                <option value="Investigación" {{ old('category') == 'Investigación' ? 'selected' : '' }}>Investigación (Conferencias, Publicaciones)</option>
                                <option value="Vida Universitaria" {{ old('category') == 'Vida Universitaria' ? 'selected' : '' }}>Vida Universitaria (Eventos, Deportes)</option>
                            </select>
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="summary" id="summary" rows="2" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>{{ old('summary') }}</textarea>
                            @error('summary') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                        </div>

                        <div class="mb-4">
                            <label for="content" class="block text-sm font-medium text-gray-700">Contenido Detallado (Opcional)</label>
                            <textarea name="content" id="content" rows="6" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('content') }}</textarea>
                            @error('content') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                        </div>

                        <div class="mb-6">
                            <label for="image" class="block text-sm font-medium text-gray-700">Imagen Destacada (Recomendado 16:9)</label>
